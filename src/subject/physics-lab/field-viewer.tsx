...
import { buildFieldViewerHtml } from '@/subject/physics-lab/field-scene-html';
import { loadFieldVendorScripts } from '@/subject/physics-lab/field-vendor';

export type FieldViewerVariant = 'inline' | 'fullscreen' | 'immersive';

type FieldViewerProps = {
  field: ParsedField;
  variant?: FieldViewerVariant;
...
export function FieldViewer({
  ...
  const sceneKey = `${field.sceneId}:${field.title}:${field.focus}:${field.labels.length}`;
  ...
  useEffect(() => {
    ...
    setError(null);
    setHtml(null);
    loadFieldVendorScripts()
      .then((vendor) => {
        if (cancelled) return;
        setHtml(buildFieldViewerHtml(field, vendor));
      })
      .catch((err: unknown) => {
        if (cancelled) return;
