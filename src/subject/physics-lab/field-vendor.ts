...
export type FieldVendorScripts = {
  three: string;
};
...
let cached: FieldVendorScripts | null = null;
...
async function loadFirst(urls: string[]): Promise<string> {
  ...
  throw lastError instanceof Error
    ? lastError
    : new Error('Could not load physics 3D libraries.');
}

export async function loadFieldVendorScripts(): Promise<FieldVendorScripts> {
