-- Rename dev test accounts from rigai.app → sheyonai.app and update password to SheyonAi-Test-2026!
...
DO $$
DECLARE
  pw text := crypt('SheyonAi-Test-2026!', gen_salt('bf'));
  old_emails text[] := ARRAY['test-google@rigai.app', 'test-apple@rigai.app'];
  new_emails text[] := ARRAY['test-google@sheyonai.app', 'test-apple@sheyonai.app'];
...
BEGIN
  FOR i IN 1..2 LOOP
    ...
    -- Update existing rigai.app account if present.
    ...
    -- Upsert sheyonai.app account (create if missing, or just update password).
